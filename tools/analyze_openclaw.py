...
from trajectory_monitor.scorer import QualityScore, score_all
...
def generate_forge_report(
    ...
    lines.append("")
    lines.append("| Job | Score | Grade | Runs | Errors | Signals |")
    lines.append("|-----|-------|-------|------|--------|---------|")
    ...
    for s in forge_scores:
        ...
        lines.append(f"| `{s.job_name}` | {s.score} | {icon} {s.grade} | {runs} | {err_rate} | {sig_count} |")
    lines.append("")
    ...
    if jobs_needing_attention:
        ...
        for s in jobs_needing_attention:
            lines.append(f"### `{s.job_name}` — score {s.score}/100")
            lines.append("")
    ...
    # Footer
    lines.append("---")
    lines.append(f"*Report generated by trajectory-monitor v0.1.0 on {now.strftime('%Y-%m-%d')}*")
...
def main() -> int:
    ...
    # JSON mode — delegate to existing reporter
    if args.json:
        report = generate_json_report(all_jobs)
        if args.output:
    ...
    # Forge-focused report
    forge_jobs = all_jobs if args.all else filter_forge_jobs(all_jobs)
    signals_by_job = analyze_all(all_jobs)
    scores = score_all(all_jobs)

    report = generate_forge_report(all_jobs, forge_jobs, signals_by_job, scores)
